...
The theory of canonical matrix forms has its genesis in the late 20th century in Jordan's original treatment of a matrix decomposition beyond diagonalzation. The history in its self has become a subject of study, for example see \cite{brechenmacher:tel-00142786}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{./images/image}
    \caption{\cite{brechenmacher:tel-00142786} page 14: A visual comparasion of the historical role of different works in matrix theory}
    \label{fig:history}
\end{figure}

The existance of the Jordan canonical form (JCF) is one of the more central results of elementary linear algebra.
This is not only because of its applications within systems theory.
But also purely algebraically as an invariant of endomorphisms on an abstract vector space under isomorphism to \( \mathbb{C}^n \).
...
And then use notions and techniques developed there to prove the Weierstrass form. Our intention is that during this process the entanglement of the two forms will emerge in a statisfying way. 
And, as the authors are algebraically predisposed, we will work through the lens of linear maps in need of a basis under which the maps' character is obvious.
...
\subsection{Prelude: Invariant subspaces} \lettrine{C}{onsider}
...
By building blocks me mean some number of subspaces, say \(k\)-many, such that if \(\beta_i\) is a basis for the \(i\)th subspace \(\vsS\) we have:
% By building blocks, we mean invariant subspaces \( \vsS_1,\vsS_2,...,\vsS_k \)
% such that given a basis \(\beta_i\) for each \(\vsS_i\), we have
\begin{align*}
    &\text{For} \quad \beta = \bigcup_{i=1}^k \beta_i  \quad \beta \ \text{is a basis for} \ \vsV \\
    &\text{Or equivilently} \quad \bigoplus_{i=1}^k\vsS_i=\vsV
\end{align*}
...
\(T-\lambda_i\id\) is nilpotent on the generalized eigenspace \(\Gi\) and therefore by Theorem \ref{theorem:multnull} we may construct an appropriate basis \(\beta_i\) - composed of nullchains - for \(\Gi\).

Define $\beta_i$ as a basis of $\Gi$ composed of null-chains and consider its matrix representation under $T$. 
