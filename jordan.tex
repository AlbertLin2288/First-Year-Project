\section{Prelude: Invariant subspaces}
\lettrine{C}{onsider}
...
\begin{definition}
    A \textit{null-chain} under a map \(L\) is a set of (one or more) \textbf{non-zero} vectors \(\{\vv_1,\vv_2,..\vv_n\}\)
    % Note:
    % Here, it's not specified that L is a linear map
    % I can't tell if it is intensional.
    % If it is, a few lemma below need to be changed to explictly require linear map (since having a null-chain won't implictly imply linear).
...
For this to first make sense, we need to make sure that that the \( \vsG_i \) are mutually disjoint, and independent - meaning the union of their bases are linearly independent.
