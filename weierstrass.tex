...
Denote $T:V\rightarrow V$ as $$T(v):=Av$$and $L:V\rightarrow V$ as $$L(v):=Bv.$$
Recalling the introduction of this paper, canonical forms are obtained by decomposing the space into invariant subspaces on which the operators haveparticularly simple matrix representations.\\
...
The problem is: such subspaces rarely exist. The maps $T$ and $L$ might be too independent of each other and therefore a subspace might act nice under one map, but arbitrarily under the other. For example, take 
\[
	T=
	\begin{pmatrix}
		1  & 0         \\
	...
	\end{pmatrix}
\]
The line $W=\text{span}(e_1)$ is $T-$invariant: $w\in W \implies T(w)=w.$\\
But, $w\in W\implies L(w)\in \text{span}(e_2)$, so $W$ is not $L-$invariant.\\ \\
In fact, for a pair $T,L$ there might be no non-trivial commonly invariant subspace.
