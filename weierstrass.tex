\section{The Weierstrass form}
\subsection{Introduction and motivaition}
The Weierstrass form is a canonical form of two matrices $A,B$ acting on the same space.
It is often referred to as generalized Jordan form. The motivation for Weierstrass form goes further than just generalizing Jordan and finding an appropriate basis - it plays crucial role in understanding linear systems of the form $$B\dot{x}=Ax$$
The relevance of such systems will be discussed in Chapter 5.\\ \\
Let $A,B\in \text{End}(V).$\\ \\
Recalling the introduction of this paper, canonical forms are obtained by decomposing the space into invariant subspaces on which the operators have particularly simple matrix representations.\\
Assume that we are able to find $k$ subspaces $S_1,S_2,...,S_k$ such that each $S_i$ is $T$ and $L$-invariant, and $$\bigoplus_{i=1}^k S_k=V.$$Picking a basis $\beta_i$ for $S_i$ and composing $\bigcup\beta_i$ gives us a block-diagonal matrix representation of $T$ \textit{and} $L$, respectively, \textit{under the same basis}.\\ \\
The problem is: such subspaces rarely exist. The maps $A$ and $B$ might be too independent of each other and therefore a subspace might act nice under one map, but arbitrarily under the other. For example, take
\[
	A=
	\begin{pmatrix}
		1  & 0         \\
		0  & 2  \\


	\end{pmatrix}
    ,
    B=
	\begin{pmatrix}
		0  & 1        \\
		1  & 0  \\


	\end{pmatrix}
\]
The line $W=\text{span}(e_1)$ is $A-$invariant: $w\in W \implies Aw=w.$\\
But, $w\in W\implies Bw\in \text{span}(e_2)$, so $W$ is not $B-$invariant.\\ \\
In fact, for a pair $A,B$ there might be no non-trivial commonly invariant subspace.\\ \\
Therefore, instead of attempting to describe the maps $A$ and $B$ via a specific basis, we do so with two different bases $\beta$ and $\gamma$ with the corresponding matrices $$_\beta[A]_ \gamma $$ $$_\beta[B]_ \gamma$$
Regarding $A$ and $B$ as matrices, this means considering two coordinate transformations: $$PAQ,\ PBQ$$where $P$ and $Q$ are invertible.\\ \\
Throughout the paper we will be treating $A$ and $B$ as linear maps, however sticking to matrix notation at times.


\subsection{Matrix pencils}
For maps $A,B\in \text{End}(V)$, our motivation is to find bases $\beta$ and $\gamma$ such that the matrices
\begin{equation}\label{form}
    _\beta[A]_ \gamma,\ _\beta[B]_ \gamma
\end{equation}
are of particularly nice type.
Prioritizing simplification, Weierstrass canonical form pursues this goal by putting a restriction on $A$ and $B$.  \\ \\
We consider pairs $(A,B)$ referred to as \textit{regular}.

\begin{definition}
    A pair of maps $(A,B)$ is called \textit{regular} iff
    \begin{equation}\label{pencil}
        \exists \lambda\in \mathbb{C}\ s.t\ |A+\lambda B|\neq0
    \end{equation}
\end{definition}

\begin{definition}
    The set $$\{A+\lambda B: \lambda\in \mathbb{C}\}$$is called a \textit{matrix pencil of the pair $(A,B)$}
\end{definition}


We say that a matrix pencil of the pair $(A,B)$ is \textit{regular} if $(A,B)$ is regular, and \textit{singular} otherwise. \\ \\
The condition (\ref{pencil}) enables us to select a non-singular element of the matrix pencil which will help us derive a canonical form of (\ref{form}).\\ \\
Before explicitly stating the form, we present necessary lemmas about matrix pencils.


\begin{lemma}
    \label{lemma:singular_from_vector}
    A matrix pencil $\penAB$ of the pair $(A,B)$ is singular if and only if for all $x\in\C$, exists $\0\neq\vv\in\vsV$ s.t. $(\penAB)\vv=\0$.
\end{lemma}
\begin{proof}
    \begin{align*}
        |\pen{A}{B}|=0
        &\iff \forall x\in\C,\, A+Bx=0\\
        &\iff \forall x\in\C,\, A+Bx \text{ is singular}\\
        &\iff \forall x\in\C,\, \exists \0\neq\vv\in\vsV,\, (A+Bx)\vv = \0\\
    \end{align*}
\end{proof}

\begin{lemma}
    \label{lemma:regular_symm}
    $A+Bx$ is regular if and only if $B+Ax$ is regular.
\end{lemma}
\begin{proof}
    Note that the forward direction is equivalent to the backward direction.

    If $A+Bx$ is regular, then there exists $x\in\C$ s.t. $|A+Bx|\neq0$.

    If $x=0$, then $A$ is invertible.
    \begin{equation*}
        |Ax+B|=|A(x\id+A\inv B)|=|A||x\id+A\inv B|
    \end{equation*}
    Pick $x\notin\spec -A\inv B$, we have $|Ax+B|\neq0$.

    Otherwise,
    \begin{equation*}
        |Ax\inv+B|=|x\inv(A+Bx)|=x^{-n}|A+Bx|\neq0
    \end{equation*}
\end{proof}

\begin{lemma}\label{lem:inject}
    \label{lemma:regular_converge_inj}
    Let $A+Bx$ be a regular matrix pencil of the pair $(A,B)$. If $B\inv(A(\vsS))=\vsS$ for some subspace $\vsS\subseteq\vsV$, then $\ker_{\vsS}A=\{\0\}$.
\end{lemma}
\begin{proof}
    Suppose $\ker_{\vsS}A\neq\{\0\}$.

    Since $B\inv(A(\vsS))=\vsS$,
    \begin{equation*}
        \im_{\vsS}B = B(\vsS) = B(B\inv(A(\vsS))) \subseteq A(\vsS)
    \end{equation*}

    Hence, for any $x\in\C$, we have
    \begin{equation*}
        \im_{\vsS}(A+Bx)\subseteq\im_{\vsS}A+\im_{\vsS}Bx=\im_{\vsS}A
    \end{equation*}
    And
    \begin{equation*}
        \dim\ker_{\vsS}(A+Bx)=\dim\vsS-\dim\im_{\vsS}(A+Bx)\ge\dim\vsS-\dim\im_{\vsS}A=\dim\ker_{\vsS}A>0
    \end{equation*}

    Therefore $A+Bx$ is singular for all $x\in\C$.
    This contradicts the assumption for the matrix pencil to be regular.

    By contradiction, $\ker_{\vsS}A=\{\0\}$
\end{proof}

We are now ready to state the desired form of (\ref{form}).












\subsection{Statement}
For a regular pair of maps $(A,B)$, there exist bases $\beta,\gamma$ such that
\begin{equation}\label{weierstrass}
\begin{aligned}
{}_{\beta}[A]_{\gamma}
&=
\begin{pmatrix}
J & 0\\
0 & I
\end{pmatrix},
\\[1em]
{}_{\beta}[B]_{\gamma}
&=
\begin{pmatrix}
I & 0\\
0 & N
\end{pmatrix}.
\end{aligned}
\end{equation}
Regarding $A$ and $B$ as matrices, equivalently: there exist invertible $P$ and $Q$ such that
\begin{equation}
PAQ=
\begin{pmatrix}
J & 0\\
0 & I
\end{pmatrix},
PBQ=
\begin{pmatrix}
I & 0\\
0 & N
\end{pmatrix},
\end{equation}



\subsection{Method}

Let $\vsV$ be a $n$-dimensional vector spaces and $\pen{A}{B}$ be a regular matrix pencil, where $A, B$ are maps from $\vsV$ to $\vsV$. Given the regularity of the pencil, we are able to choose $\lambda\in \C$ such that $(A+\lambda B)^{-1}$ is defined as a map.\\ \\
All derivations of canonical forms begin with considering decompositions of $\vsV$ into independent subspaces. For a linear map $L$, we have the following.

\begin{lemma}\label{lem:decom2}
    For an arbitrary linear map $L:\vsV\rightarrow \vsV$, $\exists N\in\mathbb{N}$ such that $$V=\ker L^N \oplus imL^N$$
\end{lemma}

\begin{proof}
  This is the same as Lemma \ref{lem:decom}. The proof is directly generalized for an arbitrary linear map $L$.
\end{proof}

Consider the well defined map $(A+\lambda B)^{-1}B:\vsV\rightarrow \vsV$. Lemma \ref{lem:decom2} implies we may pick an integer $N$ such that 

\begin{equation}\label{plus}
    V=\vsS \oplus W
\end{equation}
where $\vsS$ and $W$ are defined as
\begin{align}
    \vsS&:=\ker ((A+\lambda B)^{-1}B)^N\\
    W&:=im ((A+\lambda B)^{-1}B)^N
\end{align}
The reason for choosing to work with $(A+\lambda B)^{-1}B$ instead of just $(A+\lambda B)^{\pm1}$ is that for a vector $\vv \in \vsV$, knowing what $(A+\lambda B)^{\pm 1}\vv$ gives may still not tell us what $A\vv$ and $B\vv$ individually are, which is core in the process of deriving the canonical form in (\ref{form}). Considering $(A+\lambda B)^{-1}B$ ultimately extracts us more information about $A$ and $B$, which will be evident in a series of lemmas. First denote $$\boxed{L_{A,B}=(A+\lambda B)^{-1}B}$$and note the following usual properties.
% nzdcx
\begin{property}

\begin{enumerate}
    \item \label{first} $L_{A,B}$ is nilpotent on $S$
    \item \label{2nd} $L_{A,B}$ is invariant on $S$
    \item \label{3rd} $L_{A,B}$ is invariant on $W$
    \item \label{4th} $L_{A,B}$ is invertible on $W$
    
    
\end{enumerate}
\end{property}
\

\begin{remark}
    We could have by symmetry chosen to work with $(B+\lambda A)^{-1}A$. Then the form (\ref{form}) also alters symmetrically.
\end{remark}

\subsection{Characterising the subspaces}
A series of lemmas follows. Define $S$, $W$ and $L_{A,B}$ as in Section 4.4.
\begin{lemma}\label{inectivityA}
    $A$ is injective on $\vsS$
\end{lemma}

\begin{proof}
    We have that $L_{A,B}$ is nilpotent on $\vsS$ by definition. It can be seen that $A$ can be written as
    \begin{equation}\label{expForA}
        A=(A+\lambda B)(I-\lambda L_{A,B})
    \end{equation}

    \begin{claim}\label{cl:newinv}
        $(I-\lambda L_{A,B})$ is invariant and invertible on $\vsS$
    \end{claim}
    \begin{proof}
        From \ref{2nd}, $L_{A,B}$ is invariant on $\vsS$. Therefore $I-\lambda L_{A,B}$ is also invariant on $\vsS$ as a polynomial of $L_{A,B}$. Let $\vv\in \vsS$. $$(I-\lambda L_{A,B})\vv=0\implies \lambda L_{A,B}\vv = \vv\implies \lambda^n (L_{A,B})^n\vv=\vv\implies \vv=0 $$which by definition means $I-\lambda L_{A,B}$ is invertible.
    \end{proof}
    Having this claim, assume there is $\vu\in \vsS$ such that $A\vu=0$. We conclude the following sequence of implications. $$A\vu=0\implies (A+\lambda B)(I-\lambda L_{A,B})\vu=0\implies_{\text{invertibility of A+}\lambda B}\ (I-\lambda L_{A,B})\vu=0\implies_{\text{Claim}}\ \vu=0 $$which by the equivalence between injectivity and no non-trivial kernel, the claim follows.
    
\end{proof}

\begin{lemma}\label{injectiveB}
    $B$ is injective on $W$
\end{lemma}

\begin{proof}
    From (\ref{4th}) $L_{A,B}$ is invertible on $W$. Let $v\in W$. $$Bv=0\implies (A+\lambda B)^{-1}Bv=0\implies L_{A,B} v=0\implies v=0$$and therefore $B$ has no nontrivial kernel in $W$, hence injective.
\end{proof}



\begin{lemma}
    $A(S)\cap B(W)=\{0\}$ 
\end{lemma}
\begin{proof}
    Let $u\in A(S)\cap B(W)$. Then, by definition there are $v\in S$ and $w\in W$ such that $$u=Av=Bw$$
    As in (\ref{expForA}), we may write $A$ as $$A=(A+\lambda B)(I-\lambda L_{A,B})$$Therefore, we have $$(A+\lambda B)(I-\lambda L_{A,B})v=Bw$$which means that $$(I-\lambda L_{A,B})v=(A+\lambda B)^{-1}Bw.$$
    
    Having that $L_{A,B}$ is defined as $L_{A,B}=(A+\lambda B)^{-1}B$, we arrive at $$(I-\lambda L_{A,B})v=L_{A,B}(w)$$
    From Property \ref{3rd}, $L_{A,B}$ is invariant on $W$ and therefore $L_{A,B}(w)\in W \label{x}$.
    
    From Claim \ref{cl:newinv}, $(I-\lambda L_{A,B})$ is invariant on $S$ and therefore $(I-\lambda L_{A,B})v\in S \label{y}$

    Since $\vsV=S\oplus W$, we have that $$(I-\lambda L_{A,B})v=L_{A,B}(w)=0 $$

    But Property \ref{4th} implies $w=0$ and hence $$u=L_{A,B}(w)=0$$
\end{proof}

\begin{lemma}\label{charA}
    For $v\in S$, there exists a unique $u\in S$ such that $$Bv=Au$$
\end{lemma}

\begin{proof}
    From Claim \ref{cl:newinv}, $I-\lambda L_{A,B}$ is invertible on $S$. Therefore let $v\in S$ and choose $$u=(I-\lambda L_{A,B})^{-1}L_{A,B} (v)$$where the map $(I-\lambda L_{A,B})^{-1}$ is restricted on $S$ and hence is defined due to invertibility of $(I-\lambda L_{A,B})$ on $S$. After some computation, it can be seen that this expression for $u$ works. It is clearly unique.
\end{proof}


Analogously, the following Lemma which will not be proven holds.
\begin{lemma}\label{charB}
    For $v\in W$, there exists unique $u\in W$ such that $$Av=Bu$$
\end{lemma}


$\newline$
\subsection{Choosing bases}
With $L_{A,B}$ being nil-potent on $S$, consider a null-chain $\{v_1,...,v_k\}$ in $S$ such that $L_{A,B}(v_{1})=0\text{ and }  L_{A,B}(v_{i+1})=v_i $. By definition of $L_{A,B}$, we would have
\begin{align}
   Bv_{i+1}&=(A+\lambda B)v_i\\
   Bv_1&=0
\end{align}
However the form at (\ref{form}) indicates to instead find chains of the form 
\begin{equation}\label{chainAB}
    Bv_{i+1}=Av_{i}\text{ and }Bv_1=0
\end{equation}
Due to our previous lemmas, luckily we are able to identify such chains in $S$.\\\\
Injectivity at Lemma \ref{inectivityA} and uniqueness at Lemma \ref{charA} together imply that the the map $$A^{-1}B:S\rightarrow S$$ described as 
$$A^{-1}Bv=u \text{ where }Au=Bv$$is well defined and linear.\\
The following result is enables us to find an appropriate basis for $S$ of the form (\ref{chainAB}). 

\begin{lemma}
    The map $A^{-1}B:S\rightarrow S$ is nil-potent on $S$
\end{lemma}

\begin{proof}
    The proof of Lemma \ref{charA} states that for $v\in S$, there is unique $u\in S$ such that $Bv=Au$, where by definition of $A^{-1}B$ we have $A^{-1}Bv=u$. A closed form of $u$ is given in Lemma \ref{charA}.$$u=(I-\lambda L_{A,B})^{-1}L_{A,B} (v).$$
    Therefore,
    
    $$A^{-1}Bv=(I-\lambda L_{A,B})^{-1}L_{A,B} (v)\ \forall v\in S$$
    and hence we may write $$A^{-1}B=(I-\lambda L_{A,B})^{-1}L_{A,B}.$$
    
    By definition, we know that $L_{A,B}$ is nilpotent on $S$. 
    The nilpotency implies that $$(I-\lambda L_{A,B})^{-1}=I+\lambda L_{A,B}+\lambda^2 L_{A,B}^2+...+\lambda ^{N}L_{A,B}^{N}$$for some integer $N$ and therefore $$(I-\lambda L_{A,B})^{-1}L_{A,B}=L_{A,B}+\lambda L_{A,B}^2+...+\lambda ^{N}L_{A,B}^{N+1}.$$This is a polynomial in the nilpotent map $L_{A,B}$ with no constant term, and it is well known that it is nilpotent as well.
    
    Therefore, $A^{-1}B=(I-\lambda L_{A,B})^{-1}L_{A,B}$ is nilpotent on $S$ as desired. 
    
    
    \end{proof}

    \begin{corollary}
        There is a basis of $S$ composed of disjoint null-chains under $A^{-1}B.$
    \end{corollary}

    
    Injectivity of $B$ on $W$ from Lemma \ref{injectiveB} and the uniqueness at Lemma \ref{charB} means we can do a similar thing on $W$.\\
    The map $$B^{-1}A :W\rightarrow W$$described by $$B^{-1}Av=u \text{ where }Bu=Av$$is well defined and linear.
    



Therefore, $\dim\vsS+\dim\vsSp=n$, so $\dim\vsT+\dim\vsTp=n$.
Since $\vsT$ and $\vsTp$ are disjoint, $\vsV=\vsT\oplus\vsTp$.

Note that $A$ is injective from $\vsS$ to $\vsT$, and since $\vsT=A(\vsS)$, it is also surjective.
So we can define $A\inv:\vsT\to\vsS$ s.t. $AA\inv\vv=\vv$ and $A\inv A\vu=\vu$ for all $\vv\in\vsT$ and $\vu\in\vsS$.
Similarly, we can define $B\inv:\vsTp\to\vsSp$.

Note that
\begin{equation*}
    B(\vsS)=B(\vsS_{k+1})=B(B\inv(A(\vsS_{k})))\subseteq A(\vsS_k)=\vsT
\end{equation*}
and
\begin{equation*}
    A(\vsSp)=A(\vsSp_{k'+1})=A(A\inv(B(\vsSp_{k'})))\subseteq B(\vsSp_{k'})=\vsTp
\end{equation*}

So we can have $B':\vsS\to\vsS=A\inv\circ B$ and $A':\vsSp\to\vsSp=B\inv\circ A$, being well-defined on the domain.

Note that $B'$ is nilpotent.
\begin{proof}
    Let $\vv_k\in\vsS=\vsS_k$.

    Define $\vv_i=B'\vv_{i+1}$ for all $i\in[k]$.
    Note that
    \begin{align*}
        \vv_i&=B'\vv_{i+1}\\
        \vv_i&=A\inv B\vv_{i+1}\\
        A\vv_i&=B\vv_{i+1}\\
    \end{align*}
    So $\forall i\in[k]$, $\vv_i\in\vsS_i$.
    Notably, $\vv_0\in\vsS_0=\{\0\}$.

    Hence, $(B')^k\vv_k=\vv_0=\0$.

    Since this is true for any $\vv_k\in\vsS$, $(B')^k=0$.
    Hence, $B'$ is nilpotent.
\end{proof}

We also claim that $\spec -A'=\spec(A, B)$. Which we will show using double inclusion lemma.

\begin{proof}
    If $x\in\spec -A'$, then we have $|x\id+A'|=0$.
    Hence, $\exists\vv\in\vsSp\subseteq\vsV$ s.t. $(x\id+A')\vv=\0$.
    \begin{align*}
        (x\id+A')\vv=\0
        &\implies B(x\id+A')\vv=\0\\
        &\implies (BA'+Bx)\vv=\0\\
        &\implies (A+Bx)\vv=\0\\
        &\implies |A+Bx|=0\\
    \end{align*}

    If $x\in\spec(A, B)$, then we have $|A+Bx|=0$.
    Hence, there exists vector $\vv\in\vsV$ s.t. $(A+Bx)\vv=\0$.
    And we have $A\vv=-xB\vv$

    Note that
    \begin{equation*}
        \vsSp_{m+1}=(A+xB)\inv(B(\vsS_m))\supseteq(A+xB)\inv(B(\{\0\}))=\ker(A+xB)\ni\vv
    \end{equation*}
    Hence,
    \begin{align*}
        (x\id+A')\vv
        &=x\vv+B\inv A\vv\\
        &=x\vv-xB\inv B\vv\\
        &=\0\\
    \end{align*}
\end{proof}


Now, we can apply similar method as we did for Jordan canonical form.
For the subspace $\vsS$ and map $B'$, we can find a basis composed of null-chains.
Let
\begin{equation}
\{\vv_{0,1},\vv_{0,2},\dots,\vv_{0,m_0}\} \cup \{\vv_{1,1},\vv_{1,2},\dots,\vv_{1,m_1}\} \cup \dots \cup \{\vv_{\gamma-1,1},\vv_{\gamma-1,2},\dots,\vv_{\gamma-1,m_{\gamma-1}}\} \tag{\(\star\)} \label{eq:vbasis}
 \end{equation}
be such a basis.
% Let $\vv_0=\0$ and $\{\vv_1,\vv_2,\dots,\vv_m\}$ be such a chain.
We have $$\vv_{i,j}=B'\vv_{i,j+1}=A\inv B\vv_{i,j+1}$$.
Let $\vu_{i,j}=A\vv_{i,j}$. Since $A$ is an invertible map from $\vsS$ to $\vsT$, $\vu_{i,j}$ form a basis for $\vsT$.
We also have
\begin{equation*}
    B\vv_{i,j+1}=AA\inv B\vv_{i,j+1}=A\vv_{i,j}=\vu_{i,j}
\end{equation*}

Hence, using $\vv$ and $\vu$ as basis, we can write
\begin{align*}
    \leftindex_\vu[A]_\vv&=I\\
    \leftindex_\vu[B]_\vv&=N\\
\end{align*}
where
\vspace{2\baselineskip}
% \setcounter
\begin{equation*}
    N=
    % \begin{array}{cc c}
    %     &1&2_2\\
    %     \multirow{2}{0em}{$\left[\right.$}&&1\\
    %     &&2\\
    % \end{array}
    \begin{bNiceMatrix}
        \begin{array}{c c c c}
            0 & 1 &        &   \\
              & 0 & \ddots &   \\
              &   & \ddots & 1 \\
              &   &        & 0 \\
        \end{array}
         &&&\text{\Huge 0} \\
        &
        \begin{array}{c c c c}
            0 & 1 &        &   \\
              & 0 & \ddots &   \\
              &   & \ddots & 1 \\
              &   &        & 0 \\
        \end{array}
         && \\
        &&\ddots&\\
        \text{\Huge 0}&&&
        \begin{array}{c c c c}
            0 & 1 &        &   \\
              & 0 & \ddots &   \\
              &   & \ddots & 1 \\
              &   &        & 0 \\
        \end{array} \\
    \CodeAfter
        \OverBrace[shorten,yshift=3pt]{1-1}{1-1}{m_0}
        \OverBrace[shorten,yshift=3pt]{2-2}{2-2}{m_1}
        \OverBrace[shorten,yshift=3pt]{4-4}{4-4}{m_{\gamma-1}}
    \end{bNiceMatrix}
\end{equation*}

Similarly, for the map $A'$ on $\vsSp$, we can also find a simple basis.
However, since $A'$ is not nilpotent, we need to apply the method used for Jordan form.

We can write $\vsSp$ as the direct sum of extended eigenspace for $A'$, and find a basis for each eigenspace.
This way, for each eigen value $\lambda$, we can find chains where for each chain $\{\vv_1,\vv_2,\dots,\vv_m\}$, we have $\vv_i=(A'-\lambda\id)\vv_{i+1}$.

Let $\vu_i=B\vv_i$. With similar argument as before, $\vu$ form a basis for the eigenspace.
Note that
\begin{equation*}
    A\vv_{i+1}=BB\inv A\vv_{i+1}=BA'\vv_{i+1}=B(A'-\lambda\id)\vv_{i+1}+\lambda B\vv_{i+1}=B\vv_i+\lambda B\vv_{i+1}=\lambda\vu_{i+1} + \vu_i
\end{equation*}

Hence, using $\vv$ and $\vu$ as basis, we can write
\begin{align*}
    \leftindex_\vu[A]_\vv&=J\\
    \leftindex_\vu[B]_\vv&=I\\
\end{align*}

%     A\inv B\vu&=\lambda\vu\\
%     B\vu&=A\lambda\vu\\
% \end{align*}

% Let $m$ be the minimum integer s.t. $\vu\in\vsS_m$.
% If $m>0$, we have $\vu=\lambda^{-1}(\lambda\vu)\in\vsS_{m-1}$, creating a contradiction.
% Hence, $m=0$ and $\vu=0$.

% So the only eigenvalue of $B'$ is $0$.



% Let the eigenvalues of $T$ be $\lambda_0, \lambda_1, \dots, \lambda_{k-1}$. Let $\vecspace{S}_i=\extker T-\lambda_i\id$.

% Note that any polynomials of $T$ commute.

% % \begin{lemma}
% %     \label{lemma:closed_under_T_sub_eigen}
% %     $\forall i\in[k]$, $\vecspace{S}_i$ is closed under $T-\lambda_i\id$.
% % \end{lemma}
% % \begin{proof}
% %     $\forall \vec{u} \in \vecspace{S}_i$, $\exists m \in \N$, s.t. $(T-\lambda_i\id)^m\vec{u}=\0$. Hence, $(T-\lambda_i\id)^m(T-\lambda_i\id)\vec{u}=(T-\lambda_i\id)(T-\lambda_i\id)^m\vec{u}=\0$
% % \end{proof}

% \begin{theorem}
%     \label{theorem:closed_under_T}
%     $\forall i\in[k]$, $\vecspace{S}_i$ is closed under $T$.
% \end{theorem}
% \begin{proof}
%     $T\vec{u}=(T-\lambda_i\id)\vec{u} + \lambda_i\id\vec{u}=\lambda_i\vec{u}\in\vecspace{S}_i$
% \end{proof}

% \begin{corollary}
%     \label{corollary:closed_under_p_T}
%     $\vecspace{S}_i$ is closed under any polynomials of $T$
% \end{corollary}
% % Note that due to being a subspace, Lemma \ref{theorem:closed_under_T} imply $\vecspace{S}_i$ is closed under any polynomials of $T$.

% However, we can go one step further.
% $T-\lambda_i\id$ is a well-defined function on $\vecspace{S}_i$ thanks to \ref{corollary:closed_under_p_T}.

% \begin{lemma}
%     \label{lemma:bijection}
%     $\forall i\in[k], x\in\C$, if $x\neq\lambda_i$, $T-x\id$ is a bijection on $\vecspace{S}_i$
% \end{lemma}
% \begin{proof}
%     \begin{align*}
%         \ker_{\vecspace{S}_i}(T-x\id)
%         &=\{\vec{v}\in\vecspace{S}_i|(T-x\id)\vec{v}=\0\}\\
%         &=\{\vec{v}\in\vecspace{V}|T\vec{v}=x\vec{v} \land (T-\lambda_i\id)^{p_i}\vec{v}=\0\}\\
%     \end{align*}
%     Note that since $T\vec{v}=x\vec{v}$,
%     \begin{equation*}
%         (T-\lambda_i\id)\vec{v}=T\vec{v}-\lambda_i\vec{v}=(x-\lambda_i)\vec{v}
%     \end{equation*}
%     where $x-\lambda_i\neq0$ from the assumption. \\
%     Hence,
%     \begin{align*}
%         \ker_{\vecspace{S}_i}(T-x\id)
%         &=\{\vec{v}\in\vecspace{V}|T\vec{v}=x\vec{v} \land (x-\lambda_i)^{p_i}\vec{v}=\0\}\\
%         &\subseteq\{\vec{v}\in\vecspace{V}|(x-\lambda_i)^{p_i}\vec{v}=\0\}\\
%         &=\{\vec{v}\in\vecspace{V}|\vec{v}=\0\}\\
%         &=\{\0\}\\
%     \end{align*}
%     Since $\vecspace{S}_i$ has finite dimension, $T-x\id$ is a bijection on $\vecspace{S}_i$.
% \end{proof}

% % \begin{lemma}
% %     \label{lemma:disjoint}
% %     $\forall i\neq j \in [k]$, $\vecspace{S}_i\cup \vecspace{S}_j = \{\0\}$.
% % \end{lemma}
% % \begin{proof}
% %     Let $\vec{u}\in\vecspace{S}_i\cup\vecspace{S}_j$.
% %     Suppose $\vec{u}\neq\0$.
% %     Let $m_1 = \min\{m\in\N, (T-\lambda_i\id)^m\vec{u}=\0\}$.
% %     Note that $m_1\neq0$.
% %     Let $\vec{v}=(T-\lambda_i)^{m_1-1}\vec{u}$.
% %     Note that $\vec{v}\neq\0$, $\vec{v}\in\vecspace{S}_j$ and $(T-\lambda_i\id)\vec{v}=\0$.
% %     Hence, there exists $m_2\in\N$, s.t. $(T-\lambda_2\id)^{m_2}\vec{v}=\0$.
% %     However, $(T-\lambda_2\id)\vec{v}=(T-\lambda_1\id+(\lambda_1-\lambda_2)\id)\vec{v}=(\lambda_1-\lambda_2)\vec{v}$.
% %     Hence, $(T-\lambda_2\id)^{m_2}\vec{v}=(\lambda_1-\lambda_2)^{m_2}\vec{v}\neq\0$.
% %     By contradiction, $\vec{u}=\0$
% % \end{proof}

% % Let $p_i$ denote $\min\{p\in\N|\extker(T-\lambda_i\id) = \ker(T-\lambda_i\id)^p\}$, whose existance is shown in Lemma \ref{lemma:ext_finite_power}.

% % \begin{theorem}
% %     \label{theorem:independent}
% %     The subspaces $\vecspace{S}_0, \vecspace{S}_1, \dots, \vecspace{S}_{k-1}$ are disjoint.
% % \end{theorem}
% % \begin{proof}
% %     Suppose for $\0\neq\vec{v}_0\in\vecspace{S}_0, \dots, \0\neq\vec{v}_{k-1}\vecspace{S}_{k-1}$, there exists $x_0,\dots,x_{k-1}$ s.t. $x_0\vec{v_0}+x_1\vec{v}_1+\dots+x_{k-1}\vec{v}_{k-1}$ and $x_i\neq0$ for some $i$.

% %     WLOG, let $x_0\neq0$.
% %     \begin{align*}
% %         \0&=(T-\lambda_1\id)^{p_1}(T-\lambda_2\id)^{p_0}\dots(T-\lambda_{k-1}\id)^{p_{k-1}}\left(\sum_{i=0}^{k-1}x_i\vec{v}_i\right)\\
% %         &=(T-\lambda_1\id)^{p_1}\dots(T-\lambda_{k-1}\id)^{p_{k-1}}(x_0\vec{v}_0)+\sum_{i=1}^{k-1}(T-\lambda_1\id)^{p_1}\dots(T-\lambda_{k-1}\id)^{p_{k-1}}(x_i\vec{v}_i)\\
% %     \end{align*}
% %     Since polynomials of $T$ commute, we can rearrange $(T-\lambda_1\id)^{p_1}\dots(T-\lambda_{k-1}\id)^{p_{k-1}}(x_i\vec{v}_i)$, moving the term $(T-\lambda_i\id)^{p_{i}}$ last.
% %     Since $(T-\lambda_i\id)^{p_{i}}(x_i\vec{v_i})=\0$, the above equation imply $(T-\lambda_1\id)^{p_1}\dots(T-\lambda_{k-1}\id)^{p_{k-1}}(x_0\vec{v}_0)=\0$.

% %     However, for any $\0\neq\vec{v}\in\vecspace{S}_0$ and $i\neq0$, we have $(T-\lambda_{i}\id)^{p_i}\vec{v}\in\vecspace{S}_0$.
% %     Also, since $\vec{v}\neq\0$, from Lemma \ref{lemma:disjoint}, we know $\vec{v}\notin\vecspace{S}_i$.
% %     Therefore $(T-\lambda_{i}\id)^{p_i}\vec{v}\neq\0$.
% %     Hence, $(T-\lambda_1\id)^{p_1}\dots(T-\lambda_{k-1}\id)^{p_{k-1}}(x_0\vec{v}_0)\neq\0$, leading to contradiction.

% %     Therefore, $\vecspace{S}_0, \dots, \vecspace{S}_{k-1}$ are independent from Lemma \ref{lemma:independent_from_vectors}.
% % \end{proof}

% \begin{lemma}
%     \label{lemma:img_sum_ker}
%     $\ker(T-\lambda_i\id)^{p_i}\oplus\im(T-\lambda_i\id)^{p_i}=\vecspace{V}$
% \end{lemma}
% \begin{proof}
%     First, we will show that the two vector spaces are disjoint.

%     Suppose $\vec{v} \in \im(T-\lambda_i\id)^{p_i}$, then there exist $\vec{u}\in\vecspace{V}$ s.t. $\vec{v}=(T-\lambda_i\id)^{p_i}\vec{u}$.
%     Since $\vec{v}\in\ker(T-\lambda_i\id)^{p_i}$,
%     \begin{align*}
%         (T-\lambda_i\id)^{2p_i}\vec{u}
%         &=(T-\lambda_i\id)^{p_i}\left((T-\lambda_i\id)^{p_i}\vec{u}\right)\\
%         &=(T-\lambda_i\id)^{p_i}\vec{v}\\
%         &=\0
%     \end{align*}
%     Hence, $\vec{u}\in\extker(T-\lambda_i\id)=\ker(T-\lambda_i\id)^{p_i}$.
%     \begin{equation*}
%         \vec{v} = (T-\lambda_i\id)^{p_i}\vec{u} = \0
%     \end{equation*}
%     Therefore, $\ker(T-\lambda_i\id)^{p_i}\cap\im(T-\lambda_i\id)^{p_i}=\{\0\}$.

%     Since the two vector spaces are disjoint, they are also independent.
%     Therefore,
%     \begin{align*}
%         &\dim\left(\ker(T-\lambda_i\id)^{p_i}\oplus\im(T-\lambda_i\id)^{p_i}\right)\\
%         =&\dim\ker(T-\lambda_i\id)^{p_i} + \dim\im(T-\lambda_i\id)^{p_i}\\
%         =&\dim\vecspace{V}\\
%     \end{align*}
%     by the dimension theorem.
%     Hence, $\ker(T-\lambda_i\id)^{p_i}\oplus\im(T-\lambda_i\id)^{p_i}=\vecspace{V}$.
% \end{proof}

% \begin{theorem}
%     $\vecspace{S}_0\oplus\vecspace{S}_1\oplus\dots\oplus\vecspace{S}_{k-1}=\vecspace{V}$
% \end{theorem}
% \begin{proof}
%     Note that $\{\0\}\oplus\vecspace{S}=\vecspace{S}$. So for $k=0$, we will define $\vecspace{S}_0\oplus\dots\oplus\vecspace{S}_{k-1}=\{\0\}$.

%     If $k=0$, since for any $n$-dimensional vector space $\vecspace{V}$ where $n > 0$, there exists eigenvalues for $T$, we have $n=0$.
%     Thus, $\vecspace{V}=\{\0\}$ and the theorem holds.

%     Suppose the theorem is true for some $k-1\in\N$. \\
%     Consider the subspace $\vecspace{V}'=\im(T-\lambda_{k-1}\id)^{p_{k-1}}$.
%     Note that from Lemma \ref{lemma:img_sum_ker}, we have $\vecspace{V} = \vecspace{S}_{k-1} \oplus \vecspace{V}'$.\\
%     It is closed under $T$, as $\forall\vec{v}\in\vecspace{V}'$, there exist $\vec{u}\in\vecspace{V}$ s.t.
%     \begin{equation*}
%         \vec{v}=(T-\lambda_{k-1}\id)^{p_{k-1}}\vec{u}
%     \end{equation*}
%     Hence,
%     \begin{equation*}
%         T\vec{v} = T(T-\lambda_{k-1}\id)^{p_{k-1}}\vec{u} = (T-\lambda_{k-1}\id)^{p_{k-1}}(T\vec{u})\in\vecspace{V}'
%     \end{equation*}

%     Since $\vecspace{V}'\cap\vecspace{S}_{k-1}=\{\0\}$, $\spec_{\vecspace{V}'}T \subseteq \spec_{\vecspace{V}}T\setminus\{\lambda_{k-1}\}$.\\
%     % $\forall i\in[k-1]$, let $\0\neq\vec{x}\in\ker(T-\lambda_i\id)\subseteq\vecspace{S}_i$.
%     $\forall i\in[k-1]$, since $T-\lambda_{k-1}\id$ is a bijection on $\vecspace{S}_i$ (Lemma \ref{lemma:bijection}),
%     \begin{equation*}
%         \vecspace{S}_i=(T-\lambda_{k-1}\id)^{p_{k-1}}(\vecspace{S}_i)\subseteq\im(T-\lambda_{k-1}\id)^{p_{k-1}}=\vecspace{V}'
%     \end{equation*}

%     Hence, there are exactly $k-1$ eigen values of $T$ in $\vecspace{V}'$.
%     We can apply induction hypothesis on $\vecspace{V}'$ to get
%     \begin{equation*}
%         \vecspace{V}' = \vecspace{S}_0\oplus\vecspace{S}_1\oplus\dots\oplus\vecspace{S}_{k-2}
%     \end{equation*}

%     Therefore,
%     \begin{equation*}
%         \vecspace{V} = \vecspace{V}' \oplus \vecspace{S}_{k-1} = \vecspace{S}_0\oplus\vecspace{S}_1\oplus\dots\oplus\vecspace{S}_{k-2} \oplus \vecspace{S}_{k-1}
%     \end{equation*}

%     %  let $\0\neq\vec{x}\in\ker(T-\lambda_i\id)\subseteq\vecspace{S}_i$.

%     % need lemma to show kernel is closed under T
%     % Note that $\0\neq(T-\lambda_{k-1}\id)^{p_{k-1}}\vec{x}\in\im(T-\lambda_{k-1}\id)^{p_{k-1}}$ and $(T-\lambda_i\id)(T-\lambda_{k-1}\id)^{p_{k-1}}\vec{x}=\0$.
%     % Thus, $\lambda_i\in\spec_{\vecspace{V}'}T$.\\
%     % Therefore, we can apply the induction hypothesis on $\vecspace{V}'$. We get
%     % \begin{equation*}
%     %     \vecspace{V}' = \vecspace{S}'_0\oplus\vecspace{S}'_1\oplus\dots\oplus\vecspace{S}'_{k-2}
%     % \end{equation*}
%     % where
%     % \begin{equation*}
%     %     \vecspace{S}'_i = \extker_{\vecspace{V}'}(T-\lambda_i\id)
%     % \end{equation*}

% \end{proof}

% % \begin{lemma}
% %     \label{lemma:construction_raw}
% %     There exists linear independent subspaces $\vecspace{S_0}, \vecspace{S_1}, \dots, \vecspace{S_{k-1}}$ such that $\vecspace{V}=\vecspace{S_0} \oplus \vecspace{S_1} \oplus \dots \oplus \vecspace{S_{k-1}}$ and
% %     $\vecspace{S_i} \subseteq \extker(T-\lambda_i \id)\sin$
% % \end{lemma}
% % \begin{proof}
% %     We can construct such a list of subspaces recursively.
% %     Let $\vecspace{V_0} = \vecspace{V}$. Let $S_i$
% % \end{proof}
