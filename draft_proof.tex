...
We start with the following familiar setting:
Let $\vecspace{V} \subseteq \C^{\geq n} $ be a $n$-dimensional vector space with $n < \infty$ and $T:\vecspace{V} \to \vecspace{V}$ a linear map.

\begin{definition}
    We call $S \subseteq V$ the extended kernel of $T$, denoted $\extker_\vecspace{V}{T}$, if and only if:
\begin{equation*}
    \extker_\vecspace{V}{T} = \bigcup_{i=0}^{\infty}\ker (T^i)
\end{equation*}
\end{definition}
...
\begin{proof}
    ...
    Let $\vec{v}, \vec{u} \in \vecspace{S}$ and $k \in \C$. Then $\exists n,m \in \N$ such that $T^{n}\vec{v}=\0$, $T^{m}\vec{u}=\0$.
